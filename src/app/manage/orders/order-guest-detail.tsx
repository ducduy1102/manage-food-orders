...
import { usePayForGetMutation } from "@/queries/useOrder";
import {
  GetOrdersResType,
...
export default function OrderGuestDetail({
  ...
  const purchasedOrderFilter = guest
    ...
    : [];
  const payForGetMutation = usePayForGetMutation();

  const pay = async () => {
    if (payForGetMutation.isPending || !guest) return;
    try {
      const result = await payForGetMutation.mutateAsync({ guestId: guest.id });
