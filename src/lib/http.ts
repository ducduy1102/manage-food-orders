      ...
      if (isClient) {
        if (!clientLogoutRequest) {
          clientLogoutRequest = fetch("/api/auth/logout", {
            method: "POST",
            body: null, // Logout always allow null successfully, so don't need body
            headers: {
              ...baseHeaders,
            } as any,
          });
          try {
            ...
            // Redirect về trang login có thể dẫn đến loop vô hạn nếu ko dc xử lý đúng cách
            // Vì nếu rơi vào TH tại trang login, chúng ta có gọi các API cần accessToken
            // Mà accessToken đã bị xóa thì nó lại nhảy vào đây, và cứ thế nó sẽ bị loop
            location.href = "/login";
          }
        }
      } else {
        // Đg dùng 'user server' tức là ở phía server
